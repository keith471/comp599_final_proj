...
def print_results(results):
    ...
    for max_nb_words, res_0, res_1 in results:
        print('%d\t%d\t%f\t%f\t%d\t%f\t%f' % (max_nb_words, res_0[0], res_0[1], res_0[2], res_1[0], res_1[1], res_1[2]))
    print()
...
if __name__ == '__main__':
    ...
    results = []
    #for max_nb_words in [500, 1000, 2000, 5000, 10000, 15000, 20000, 25000, 30000]:
    for max_nb_words in [500]:
        curr_results = [max_nb_words]
        for max_sequence_length in [1000, 8066]:
            print('-' * 80)
            ...
            for i in range(3):
                ...
                model.fit(X_train, curr_y_train, validation_data=(X_val, y_val),
                          ...
                          callbacks=[EarlyStopping(monitor='val_loss', patience=0, verbose=True)])
                print()
    ...
    # convert the test data into the proper form and get test results
    texts_test = preproc(all_unproc_X_test)
    X_test = tokenize_test_texts(texts_test, tokenizer)

    y_test = to_categorical(np.asarray(y_test))
    print('Shape of X_test tensor:', data.shape)
    print('Shape of y_test tensor:', y_test.shape)
    print()
    ...
    to_pickle('cnn_loss_and_metrics', loss_and_metrics, with_time=False)
    ...
    # pretokenize the text yourself, but then string it back together and pass it to the tokenizer, which will simply split using " "
    tokenized_texts = []
    wnl = WordNetLemmatizer()
    for doc in all_unproc_X_train:
        nopunc = ''.join([i if i not in string.punctuation else '' for i in doc])
        tokens =  word_tokenize(nopunc)
        tokens = [w.lower() for w in tokens]
        tokens = [w for w in tokens if w not in stopwords.words('english')]
